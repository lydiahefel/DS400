...
```{r}
ggplot(running, aes(y = net, x = age, color = runner)) +
  geom_point()
```

```{r}
ggplot(running, aes(y = age, x = runner, color = net)) +
  geom_point()
```

```{r}
ggplot(running, aes(y = net, x = runner, color = age)) +
  geom_point()
```
...
-   <div>
    ...
    1.  Below 75 minutes, at 62 minutes
