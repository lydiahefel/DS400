...
```{r}
ggplot(bikes, aes(x = temp_actual, y = rides, color = weather_cat)) +
  geom_point() +
...
```{r}
ggplot(bikes, aes(x = temp_feel, y = rides, color = weather_cat, shape = weather_cat)) +
  geom_point() +
  facet_wrap(~weather_cat) +
  geom_smooth(method = "lm") +
  stat_cor()
...
```{r}
ggplot(bikes, aes(x = rides, fill = holiday)) +
  geom_histogram()
```

```{r}
ggplot(bikes, aes(x = rides, fill = weather_cat)) +
  geom_histogram()
  
```

```{r}
ggplot(bikes, aes(x = date, y = rides, color = weekend, shape = holiday)) +
  geom_point()
```

```{r}
ggplot(bikes, aes(x = day_of_week, y = rides, fill = as.factor(year))) +
  geom_col() +
  facet_wrap(~season)
```
...
```{r}
prediction <- 
  posterior_predict(bike_model_all, newdata = new_best_day_data)
```

```{r}
mcmc_dens(prediction) + 
  xlab("new prediction of rides for the best day")
```

```{r}
new_worst_day_data <- data.frame(
  temp_feel = 29, 
  temp_actual = 35,
  season = "winter",      
  year = 2011,           
...
```{r}
prediction <- 
  posterior_predict(bike_model_all, newdata = new_worst_day_data)
```

```{r}
mcmc_dens(prediction) + 
  xlab("new prediction of rides for the worst day")
```
