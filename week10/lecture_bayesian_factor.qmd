...
```{r}
ggplot(bikes, aes(x = weekend, y = rides, fill = weekend)) +
  geom_boxplot() +
...
```{r}
ttestBF(formula = rides ~ weekend,  # Numeric variable on left, categorical on right
        data = bikes)
```
...
df %>% 
  group_by(DRD370B) %>% 
  summarise(mean_LBXBGM = mean(LBXBGM, na.rm = TRUE))
```
...
```{r}
ttestBF(formula = LBXBGM ~ DRD370B,
        data = df)
```
