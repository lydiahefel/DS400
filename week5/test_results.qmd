...
The probability that a person has the disease. P(B) = 0.001
...
The likelihood that a person has the disease given that they have a positive test result. L(B\|A) = 1.0

The probability that a person has a positive test result given they have the disease. P(A\|B) = 1.0
...
```{r}
likelihood_has_disease_given_tests_positve <- 1.0
```

#### P(A)

The probability that a person has a positive test result. P(A) = 0.0112
...
```{r}
probability_tests_positive <- 0.0112
```

#### P(B\|A)

The probability of having the disease given a positive test result. P(B\|A) = 0.1

```{r}
(probability_has_disease * likelihood_has_disease_given_tests_positve) / probability_tests_positive
```

```{r}
posterior_probability_has_disease_given_tests_positive <- 0.089
```
...
What other information is needed to answer this question?

-   The sample, or population, should consist of patients who have taken a second test after testing positive the first time.

-   The now prior probability of having the disease would be the original posterior probability.

-   The now posterior probability of the second test would be much greater than the original posterior probability.
