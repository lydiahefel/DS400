...
```{r}
likelihood_article_fake_has_excl <- 0.26
```
...
```{r}
(prior_probability_article_fake * likelihood_article_fake_has_excl) / prior_probability_has_excl
```
...
```{r}
pop_vs_soda_limited <- pop_vs_soda %>%
  select(region, pop)

pop_vs_soda_limited <- pop_vs_soda_limited %>%
  group_by_all() %>%
  count()
```
...
1\) Build a prior probability model (an unconditional probability model) with probabilities for where the person is from \*the following was done based on the pop_vs_soda data rather than the census data
...
Marginal Prior Probability - a person uses the word "pop"
...
Posterior Probability P(S\|A) - a person is from the south given they use the word "pop"
